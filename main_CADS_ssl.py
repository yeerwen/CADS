...
def train_CADS(args):
    ...
    shutil.copyfile("main_CADS_ssl.py", os.path.join(args.output_dir, "code", "main_CADS_ssl.py"))
    ...
    shutil.copyfile("models/PVTv2.py", os.path.join(args.output_dir, "code", "PVTv2.py"))
    ...
    elif args.arch == "pvtv2_b1":
        student = pvt_v2_b1()
        teacher = pvt_v2_b1_tea()
    else:
    ...
    student = utils.MultiCropWrapper_v5(student,
                ...
                DINOHead(128, 60000, args.use_bn_in_head),
                DINOHead(320, 60000, args.use_bn_in_head),
    ...
    for epoch in range(start_epoch, args.epochs): #args.epochs
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, cads_loss,
        ...
        save_dict = {
            ...
            'epoch': epoch + 1,
            'cads_loss': cads_loss.state_dict(),
        }
...
def train_one_epoch(student, teacher, teacher_without_ddp, cads_loss, data_loader,
                    optimizer, lr_schedule, wd_schedule, momentum_schedule,epoch,
                    fp16_scaler, args):

    now_it = 0
    average_loss = {"total_loss":[], "loss_ssl":[], "loss_self_1":[],  "loss_self_2":[],  "loss_self_3":[]}
    ...
    for images in data_loader:
        ...
        # print(images[0].size(), images[1].size(), images[2].size())

        with torch.cuda.amp.autocast(fp16_scaler is not None):

            teacher_output = teacher(images[:2])  # only the 2 global views pass through the teacher
            student_output = student(images)
            loss, loss_ssl, loss_self_1, loss_self_2, loss_self_3, loss_item = cads_loss(student_output, teacher_output, epoch)
            average_loss["total_loss"].append(loss.item())
            average_loss["loss_ssl"].append(loss_ssl.item())
            average_loss["loss_self_1"].append(loss_self_1.item())
            average_loss["loss_self_2"].append(loss_self_2.item())
            average_loss["loss_self_3"].append(loss_self_3.item())
        ...
        if now_it % 100 == 0:
            print("[{}/{}]\t[{}/{}]\ttotal_loss:{}\tloss_1:{}\tloss_2:{}\tloss_3:{}\tloss_4:{}\tlr:{}\tw:{}".format(epoch, args.epochs, now_it, len(data_loader), round(loss.item(),4), loss_item[0], loss_item[1], loss_item[2], loss_item[3], round(optimizer.param_groups[0]["lr"],6), round(optimizer.param_groups[0]["weight_decay"],4)))
...
class CADSLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        teacher_out_self_4 = F.softmax((teacher_output - self.center) / temp, dim=-1).detach().chunk(2)

        ssl_loss = 0
        loss_item_1, loss_item_2, loss_item_3, loss_item_4 = [], [], [], []
        self_loss_1 = 0
        self_loss_2 = 0
        self_loss_3 = 0
        n_loss_terms = 0
        for iq in  range(len(teacher_out_self_4)):
            for v in range(len(student_out_4)):
                ...
                self_loss_3 += loss_3
                ssl_loss += loss_4
                loss_item_1.append(round(loss_1.item(), 4))
                loss_item_2.append(round(loss_2.item(), 4))
                loss_item_3.append(round(loss_3.item(), 4))
                loss_item_4.append(round(loss_4.item(), 4))
                n_loss_terms += 1
        ...
        return total_loss, ssl_loss, self_loss_1, self_loss_2, self_loss_3, [loss_item_1, loss_item_2, loss_item_3, loss_item_4]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('CADS', parents=[get_args_parser()])
